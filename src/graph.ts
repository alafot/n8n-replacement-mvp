...
export type NodeType = 'httpRequest' | 'code' | 'transform' | 'htmlExtract' | 'if' | 'switch' | 'filter' | 'merge' | 'loop' | 'wait' | 'noop' | 'stopError' | 'executeSubworkflow' | 'aggregate' | 'splitOut' | 'sort' | 'limit' | 'removeDuplicates' | 'renameKeys' | 'dateTime' | 'summarize' | 'compareDatasets' | 'scheduleTrigger' | 'webhookTrigger' | 'respondToWebhook' | 'formTrigger' | 'errorTrigger';
