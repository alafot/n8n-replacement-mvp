...
export type NodeType = 'httpRequest' | 'code' | 'transform' | 'htmlExtract' | 'xml' | 'markdown' | 'crypto' | 'graphql' | 'rssRead' | 'if' | 'switch' | 'filter' | 'merge' | 'loop' | 'wait' | 'noop' | 'stopError' | 'executeSubworkflow' | 'aggregate' | 'splitOut' | 'sort' | 'limit' | 'removeDuplicates' | 'renameKeys' | 'dateTime' | 'summarize' | 'compareDatasets' | 'scheduleTrigger' | 'webhookTrigger' | 'respondToWebhook' | 'formTrigger' | 'errorTrigger';
