...
export type NodeType = 'httpRequest' | 'code' | 'transform' | 'if' | 'switch' | 'filter' | 'merge' | 'loop' | 'wait' | 'noop' | 'stopError' | 'executeSubworkflow' | 'aggregate' | 'splitOut' | 'sort' | 'limit' | 'removeDuplicates' | 'renameKeys' | 'dateTime' | 'summarize' | 'compareDatasets' | 'scheduleTrigger' | 'webhookTrigger' | 'respondToWebhook';
